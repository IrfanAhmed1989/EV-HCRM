...
class Db:
    ...
    def connect(self, host=self.cfg.get("host"), port=int(self.cfg.get("port") or 3306), user=self.cfg.get("user"), password=self.cfg.get("password"), database='ev_hcrm'):
        ...
        for sock in sockets:
            try:
                self.conn = mysql.connector.connect(
                    unix_socket=sock, user=self.cfg.get("user"), password=self.cfg.get("password"),
        ...
        try:
            self.conn = mysql.connector.connect(
                host=self.cfg.get("host"), port=int(port), user=self.cfg.get("user"), password=self.cfg.get("password"),
                database=database, connection_timeout=5
            )
            return
        except Exception as e:
...
class DAL:
    ...
    def connect(self, host=self.cfg.get("host"), port=int(self.cfg.get("port") or 3306), user=self.cfg.get("user"), password=self.cfg.get("password"), database='ev_hcrm'):
        self.db.connect(host=self.cfg.get("host"), port=port, user=self.cfg.get("user"), password=self.cfg.get("password"), database=database)
